...
class TestOrbitSim(unittest.TestCase):
    ...
    def testOrbitSimLoad(self):
        ...
        self.assertNotEqual(
            OrbitSim("test_json/test_orbits/happy_case.json")._nodes, []
        )
        self.assertNotEqual(
            OrbitSim("test_json/test_orbits/happy_case.json")._nodes, {}
        )
        self.assertEqual(
            len(OrbitSim("test_json/test_orbits/happy_case.json")._nodes), 4
        )
        self.assertEqual(
            len(OrbitSim("test_json/test_orbits/happy_case.json")._links), 3
        )
...
class TestOrbitSimTick(unittest.TestCase):
    ...
    def testOrbitSimTickUpMotion(self):
        self.os.createTrajectory(
            targetId=1, particleId=1, initialState=TrajectoryState.ACTIVE
        )
        self.os.transitParticle(1, 0)
        self.os.tick(100)
    ...
    def testOrbitSimTickDownMotion(self):
        ...
        self.os.createTrajectory(
            particleId=5, targetId=0, initialState=TrajectoryState.ACTIVE
        )
        self.os.transitParticle(5, 0)
        self.os.tick(300)
...
class TestOrbitSimParticleArrivalSurface(unittest.TestCase):
    ...
    def testOrbitSimSurfaceArrival(self):
        ...
        self.os.createTrajectory(
            "MES",
            3,
            surfaceCoordinates=SurfacePoint(50, 50),
            initialState=TrajectoryState.PENDING,
        )
        self.os.tick(1000)
        ...
        self.assertEqual(
            self.surface.objectsAtPoint(SurfacePoint(50, 50))[0].name, "ISS Meghalaya"
        )
...
class TestOrbitSimTrajectoryState(unittest.TestCase):
    ...
    def testOrbitSimTrajectoryDefinition(self):
        ...
        self.assertEqual(
            self.os.trajectoryForParticle(0).state, TrajectoryState.DEFINITION
        )
        self.os.tick(1e6)
    ...
    def testOrbitSimTrajectoryPending(self):
        self.os.createTrajectory(
            1, sourceId=0, particleId=0, initialState=TrajectoryState.PENDING
        )
        self.assertEqual(
            self.os.trajectoryForParticle(0).state, TrajectoryState.PENDING
        )
        self.os.tick(100)
    ...
    def testOrbitSimTrajectoryActive(self):
        self.os.createTrajectory(
            1, sourceId=0, particleId=0, initialState=TrajectoryState.ACTIVE
        )
        self.assertEqual(self.os.trajectoryForParticle(0).state, TrajectoryState.ACTIVE)
        self.os.tick(100)
        self.assertFalse(0 in self.os.nodeById(0).particles)
    ...
    def testOrbitSimTrajectoryComplete(self):
        self.os.createTrajectory(
            1, sourceId=0, particleId=0, initialState=TrajectoryState.COMPLETE
        )
        self.assertEqual(
            self.os.trajectoryForParticle(0).state, TrajectoryState.COMPLETE
        )
        self.os.tick(100)
        with self.assertRaises(KeyError):
...
class TestOrbitSimFindPath(unittest.TestCase):
    ...
    def testOrbitSimLoops(self):
        self.assertEqual(
            self.los._findPath(0, 3, []), [[0, 0, 1, 2, 2, 3, 3], [0, 1, 3]]
        )
        self.assertEqual(
            self.los._findPath(3, 4, []), [[3, 4, 4], [3, 5, 4], [3, 6, 4]]
        )
        self.assertEqual(
            self.los._findPath(0, 4, []),
            [
                [0, 0, 1, 2, 2, 3, 3, 4, 4],
                [0, 0, 1, 2, 2, 3, 3, 5, 4],
                [0, 0, 1, 2, 2, 3, 3, 6, 4],
    ...
    def testOrbitSimUpDown(self):
        self.assertEqual(
            self.udos._findPath(0, 10, []),
            [[0, 0, 1, 1, 2, 2, 3, 4, 7, 7, 8, 9, 9, 10, 10]],
        )
        self.assertEqual(
            self.udos._findPath(6, 10, []),
            [[6, 6, 5, 5, 4, 3, 3, 4, 7, 7, 8, 9, 9, 10, 10]],
        )
...
class TestOrbitSimTrajectory(unittest.TestCase):
    ...
    def testOrbitSimTrajectoryCreationExistingParticle(self):
        ...
        self.assertIsInstance(
            self.os.createTrajectory(targetId=2, particleId=1), OrbitTrajectory
        )
        self.assertEqual(
            self.os.createTrajectory(targetId=2, particleId=2).particleId, 2
        )
    ...
    def testOrbitSimTrajectory1Hop(self):
        self.assertEqual(
            self.os.createTrajectory(targetId=1, particleId=0).trajectory, [0, 0, 1]
        )

    def testOrbitSimTrajectoryNoncontiguous(self):
        self.assertEqual(
            self.ucos.createTrajectory(
                targetId=2, sourceId=0, payload=PayloadMock(100)
            ).trajectory,
            [0, 0, 1, 1, 2],
        )
        self.assertEqual(
            self.ucos.createTrajectory(
                targetId=4, sourceId=3, payload=PayloadMock(100)
            ).trajectory,
            [3, 2, 4],
        )
        with self.assertRaises(ValueError):
    ...
    def testOrbitSimTrajectoryDeltaV(self):
        self.assertEqual(
            self.los.createTrajectory(
                targetId=3, sourceId=0, payload=PayloadMock(100)
            ).trajectory,
            [0, 1, 3],
        )
    ...
    def testOrbitSimTrajectoryTimeStep(self):
        self.os.createTrajectory(
            targetId=1, particleId=0, initialState=TrajectoryState.ACTIVE
        )
        self.os.tick(100)
    ...
    def testOrbitSimMultiHop(self):
        self.udos.createTrajectory(
            targetId=9,
            sourceId=0,
            payload=PayloadMock(10000.0),
            initialState=TrajectoryState.ACTIVE,
        )
        self.udos.tick(1000)
...
class TestOrbitSimDeltaVBudget(unittest.TestCase):
    ...
    def testOrbitSimDeltaVInsufficientDeltaV(self):
        self.udos.createTrajectory(
            10, particleId=3, initialState=TrajectoryState.ACTIVE
        )
