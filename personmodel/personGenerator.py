...
class PersonGenerator:
    def __init__(self, path = "test_json/test_person"):
        (surnameFile, maleFile, femaleFile) = (open(path+fn, "r") for fn in ("/surnames.json", "/male_names.json", "/female_names.json"))
        (surnameJson, maleJson, femaleJson) = (json.load(f) for f in (surnameFile, maleFile, femaleFile))
        (self.surnames, self.male_names, self.female_names) = (j["Names"] for j in (surnameJson, maleJson, femaleJson))
