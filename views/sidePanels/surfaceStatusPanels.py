...
class RegionStatusPanel(SideStatusPanel):
    def __init__(self, rect, planet, manager=None, model=None):
        ...
        self.model = model
        self.planet = planet
        self.region_name_label = UILabel(
        ...
        self.region_text = UITextBox(
            region_text, (0, 200, 400, 200), manager=manager, container=self.container
        )
    ...
    def update(self):
        ...
        terrain = self.model.planetSim.planetClassById(self.planet.surface.planetClass)[
            self.region.terrain
        ]
        ...
        Radiation: not implemented""".format(
                terrain.name
            )
        )
    ...
    def update(self):
        ...
        if isinstance(self.vehicle, SurfaceVehicle) and self.vehicle.destination:
            ...
            Destination: {1}""".format(
                    self.vehicle.point, destination_text
                )
            )
