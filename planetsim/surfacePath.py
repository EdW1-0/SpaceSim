...
class SurfacePath:
    ...
    def pointOnPath(self, point):
        if self.p1.longitude > self.p2.longitude:
            ...
            if (
                point.longitude > self.p1.longitude
                or point.longitude < self.p2.longitude
            ):
                return True
        else:
            return (
                point.longitude > self.p1.longitude
                and point.longitude < self.p2.longitude
            )
    ...
    def intermediatePointTrig(self, fraction):
        ...
        x = a * math.cos(p1latR) * math.cos(p1longR) + b * math.cos(p2latR) * math.cos(
            p2longR
        )
        y = a * math.cos(p1latR) * math.sin(p1longR) + b * math.cos(p2latR) * math.sin(
            p2longR
        )
...
def pathsIntersect(path1, path2):
    intersections = tuple(
        pointFromVector(i).canonical() for i in gcIntersections(path1, path2)
    )
    intersect = False
                ...
                if math.isclose(path.p1.longitude, path.p2.longitude):
                    ...
                    if path.isDoublePolar():
                        if not isIntermediate(
                            i.latitude, (path.p1.latitude, path.p2.latitude)
                        ):
